...
class ExportRequest(BaseModel):
    ...
    format: Optional[str] = "polygon"  # "polygon" (CVAT compatible) or "rle"
...
def mask_to_polygon(mask: np.ndarray, simplify_tolerance: float = 1.0) -> List[List[float]]:
    ...
    for contour in contours:
        ...
        if len(contour) >= 3:
            # Convert contour to [x1,y1,x2,y2,...] format
            polygon = []
...
def convert_to_coco_format(export_data: ExportRequest, use_polygon: bool = True) -> Dict[str, Any]:
    ...
    # Process images using original file names (required for CVAT matching)
    image_id_map = {}
    ...
    # Process categories using original names (for CVAT label matching)
    ...
    for ann_idx, ann in enumerate(export_data.annotations, start=1):
        ...
        # Prefer category_name to resolve category_id (more reliable)
        ...
        # Select conversion mode based on requested format
        if use_polygon:
            # Convert to polygon format (CVAT default)
        ...
        coco_ann = {
            ...
            "area": float(ann.area),
            "iscrowd": iscrowd,
            "attributes": {"occluded": False}  # CVAT default attributes
        }
